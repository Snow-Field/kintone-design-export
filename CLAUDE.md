...
- **列幅は指定しない。** 見出しと値の表示幅（全角 2・半角 1）から `computeColumnWidths` が自動で決める。xlsx には「開いたときに自動調整せよ」という指定が無いため、Excel の列幅自動調整に相当する結果をこちらで計算している。
- `title` を持つ表は表題行が上に付く。プロセス管理のように 1 シートへ表を 2 つ並べる場合の区別に使う。表が 1 つのシートでは付けない。
